...
│   ├── setup-session-host-hardening.sh # Security + Teams + screen lock
...
│   ├── setup-avd-alerts.sh             # Azure Monitor alerts + budget
│   ├── provision-avd-users.sh          # Onboard new consultants
...
│   └── user-provisioning-intake.json   # New user intake form
...
| `setup-avd-alerts.sh` | Email alerts + budget monitoring | `./setup-avd-alerts.sh --resource-group <rg> --email <email> --budget-amount 220` |

### User Management
| Script | Purpose | Usage |
|--------|---------|-------|
| `provision-avd-users.sh` | Onboard new consultants | `./provision-avd-users.sh users.json` or `--single --first John --last Doe --email john@gmail.com --customer Customer-001` |

---

## 👥 Adding New Users

When new consultants join, use the provisioning workflow:

```bash
# Option 1: Single user
./provision-avd-users.sh --single \
  --first "John" \
  --last "Doe" \
  --email "john.doe@gmail.com" \
  --customer "Customer-001"

# Option 2: Bulk from JSON file
# Edit templates/user-provisioning-intake.json with user details
...
**What gets created:**
- Entra ID account (e.g., john.doe@yourdomain.onmicrosoft.com)
- Added to AVD Users group (desktop access)
- Added to customer security group
- M365 Business Premium license assigned
- Temporary password saved to `provisioned-users.txt`
