# TKT Philippines AVD Platform V6.2
...
cd tkt-azure-platform/azure-platform/tkt-azure-platform-v6.2
...
| Feature | Description |
|---------|-------------|
| 🔐 **Entra ID Join** | Cloud-only authentication (no on-prem AD required) |
| 🆔 **Managed Identity** | VMs created with system-assigned identity |
| 🔑 **VM Login RBAC** | Virtual Machine User Login role auto-assigned |
| ⏱️ **Extended Wait** | 90-second wait for Entra ID join completion |

### V6.2 Fixes Critical Issue
Previous versions created VMs without Entra ID join, causing "DomainJoinedCheck failed" errors. V6.2 includes:
- `AADLoginForWindows` extension installation
- `aadJoin: true` parameter in DSC configuration
- `Virtual Machine User Login` RBAC assignment
