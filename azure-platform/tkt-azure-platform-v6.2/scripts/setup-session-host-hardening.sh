...
'@
    ...
    #---------------------------------------------------------------------------
    # 9. INSTALL MICROSOFT TEAMS WITH AVD OPTIMIZATION
    #---------------------------------------------------------------------------
    Write-Log "Installing Microsoft Teams with AVD media optimization..."
    
    # Set registry key to enable AVD media optimization
    ...
    # Download and install Teams new (Teams 2.0)
    $TeamsUrl = "https://go.microsoft.com/fwlink/?linkid=2243204"
    $TeamsInstaller = "C:\Temp\TeamsBootstrapper.exe"
    
    try {
        Invoke-WebRequest -Uri $TeamsUrl -OutFile $TeamsInstaller -UseBasicParsing
        
        # Install Teams in per-machine mode for AVD
        Start-Process -FilePath $TeamsInstaller -ArgumentList "-p" -Wait -NoNewWindow
        
        Write-Log "Microsoft Teams installed with AVD optimization" "SUCCESS"
    } catch {
        Write-Log "Teams installation skipped - may need manual install: $_" "WARN"
    }
    
    # Enable WebRTC redirection for Teams
    $RDPRegPath = "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services"
    Set-ItemProperty -Path $RDPRegPath -Name "fEnableMediaOptimization" -Value 1 -Type DWord -ErrorAction SilentlyContinue
    
    Write-Log "Teams multimedia redirection enabled" "SUCCESS"
    
    #---------------------------------------------------------------------------
    # 10. SCREEN LOCK / IDLE TIMEOUT (15 minutes)
    #---------------------------------------------------------------------------
    Write-Log "Configuring screen lock after 15 minutes idle..."
    ...
    # Enable screen saver and lock
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" `
        -Name "ScreenSaveTimeOut" -Value "900" -Type String -ErrorAction SilentlyContinue
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" `
        -Name "ScreenSaverIsSecure" -Value "1" -Type String -ErrorAction SilentlyContinue
    Set-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\Control Panel\Desktop" `
        -Name "ScreenSaveActive" -Value "1" -Type String -ErrorAction SilentlyContinue
    
    # Also set via Group Policy path
    $DesktopPolicyPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"
    Set-ItemProperty -Path $DesktopPolicyPath -Name "InactivityTimeoutSecs" -Value 900 -Type DWord -ErrorAction SilentlyContinue
    
    Write-Log "Screen lock after 15 minutes configured" "SUCCESS"
    
    #---------------------------------------------------------------------------
    # 11. INSTALL AZURE MONITOR AGENT
    #---------------------------------------------------------------------------
