...
#     (v7 session logging used 0-indexed naming targeting non-existent VMs)
#   - CRITICAL FIX: FSLogix share name corrected to 'profiles' (not 'fslogix-profiles')
#   - CRITICAL FIX: VM schedule weekday filtering now actually implemented
#   - CRITICAL FIX: Storage keys never printed to console
#   - CRITICAL FIX: Passwords hidden from process listings (uses temp files)
#   - NEW: VM size upgraded to Standard_D4s_v5 (4 vCPU, 16GB RAM)
#   - NEW: MaxSessionLimit = 2 (2 concurrent users per VM, not 4)
#   - NEW: Teams optimization (WebRTC, multimedia redirection, AVD env flag)
#   - NEW: Shared documentation storage ('shared-docs' Azure Files share, 50GB)
#   - NEW: Browser optimization (Edge enterprise with Fiori/Zoho bookmarks)
...
#   - NEW: Encryption at Host enabled on session host VMs
#   - NEW: Phase 5.5 Security & Governance:
#       * CanNotDelete resource lock on resource group
#       * Diagnostic settings on host pool and workspace (all AVD categories)
#       * Autoscale scaling plan with business hours schedule (Asia/Manila)
#       * Watermarking via Group Policy (financial data protection)
#       * Screen capture protection on session hosts
#       * Azure Service Health alert for Southeast Asia
#       * Cost budget alert (EUR 350/month, 80% and 100% thresholds)
#   - UPDATED: Validation checks for Trusted Launch, resource lock, diagnostics, scaling plan
#
# CONTEXT:
#   Deploys Azure Virtual Desktop for 4 SAP consultants providing managed
...
#
# COST: ~EUR280/month (2x D4s_v5 session hosts + shared storage)
#===============================================================================
...
show_banner() {
    ...
    echo -e "${BLUE}|        ╚═╝   ╚═╝  ╚═╝   ╚═╝        ╚═╝     ╚═╝  ╚═╝                           |${NC}"
    echo -e "${BLUE}|                                                                               |${NC}"
    echo -e "${BLUE}|              Azure Virtual Desktop - V8 Automated Deployment                  |${NC}"
...
show_config_summary() {
    echo ""
    echo -e "${CYAN}===============================================================================${NC}"
    echo -e "${CYAN}  DEPLOYMENT SUMMARY - V8${NC}"
    echo -e "${CYAN}===============================================================================${NC}"
    echo ""
    echo "  Azure"
    echo "  -----"
...
deploy_phase5_identity() {
    ...
    # Create users
    # V8 FIX: Password passed via temp file, never in process listing
...
deploy_phase5_5_security_governance() {
    ...
    for i in $(seq 1 $VM_COUNT); do
        ...
        az vm run-command invoke \
            ...
            --scripts '
                ...
                Write-EventLog -LogName Application -Source "TKT-Platform" -EventId 8020 -EntryType Information -Message "TKT V8: Watermarking and screen capture protection configured"
    ...
    # -----------------------------------------------------------------
    # 5.5.7: Cost Budget Alert (EUR 350/month)
    # -----------------------------------------------------------------
    log INFO "Creating cost budget alert (EUR 350/month)..."
    ...
    log SUCCESS "Cost budget created: EUR 350/month (80% and 100% alerts to $ALERT_EMAIL)"
...
show_summary() {
    echo ""
    echo -e "${GREEN}+===============================================================================+${NC}"
    echo -e "${GREEN}|                       DEPLOYMENT COMPLETE (V8)                                 |${NC}"
    echo -e "${GREEN}+===============================================================================+${NC}"
    echo ""
    ...
    echo "  Join Type:          Microsoft Entra ID (cloud-only)"
    echo "  Estimated Cost:     ~EUR280/month"
    echo ""
    echo "  +-------------------------------------------------------------------------+"
    echo "  |  USER ACCESS                                                            |"
    echo "  +-------------------------------------------------------------------------+"
    ...
    echo "    - Diagnostic settings on host pool and workspace"
    ...
    echo "    - Service Health alert (Southeast Asia)"
    echo "    - Cost budget alert (EUR 350/month, 80%/100% thresholds)"
    echo ""
    echo "  Session Logging:"
    echo "    - Enhanced Windows audit policies"
    echo "    - Azure Monitor Agent with Data Collection Rules"
    echo "    - Weekly JSON export to $SHARED_DOCS_SHARE_NAME (Sundays 23:00)"
    echo "    - Report includes: sessions, apps, websites, Teams stats"
    echo ""
    echo "  Shared Storage Structure (Z:\\):"
    echo "    Z:\\SOPs\\P2P\\            - Procurement process SOPs"
    echo "    Z:\\SOPs\\R2R\\            - Record-to-Report SOPs"
    echo "    Z:\\SOPs\\cross-functional\\ - Cross-functional SOPs"
    echo "    Z:\\knowledge-base\\      - Knowledge base articles"
    echo "    Z:\\templates\\           - Document templates"
    echo "    Z:\\weekly-reports\\      - Auto-generated session reports"
    echo ""
    echo "  Files:"
    echo "    Log:               $LOG_FILE"
    echo "    Registration:      $REGISTRATION_TOKEN_FILE"
    echo ""
    echo -e "  ${YELLOW}V7 Bug Fixes Applied in V8:${NC}"
    echo "    [1] VM naming: 1-indexed with zero-padding everywhere"
    echo "    [2] FSLogix share: 'profiles' (not 'fslogix-profiles')"
    echo "    [3] VM schedule: weekday filtering implemented"
    echo "    [4] Storage keys: never printed to console"
    echo "    [5] Passwords: not visible in process listings"
    echo ""
    ...
    echo "    5. Verify SAP Fiori access via Edge on session host"
    echo "    6. Verify Zoho Desk access via Edge on session host"
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --help|-h)
                ...
                echo ""
                echo "TKT Azure Platform V8 - AVD Deployment Script"
                echo ""
                ...
                echo "  VM_COUNT         Number of session hosts (default: 2)"
                ...
                echo ""
                echo "Cost: ~EUR280/month (2x D4s_v5 + shared storage)"
                echo "Version: $VERSION_TAG"
                exit 0
                ;;
    ...
    # V8 FIX: Setup password temp files (hidden from process listings)
    setup_password_files
