...
# Date: 2026-02-17
#
# Part of Track B (Contributor) — run AFTER deploy-infra.sh
#
# PURPOSE:
...
#   - deploy-infra.sh must have been run first
#   - bash shell (not zsh)
#
# USAGE:
...
#===============================================================================
# DEFAULT CONFIGURATION - Must match deploy-infra.sh values
#===============================================================================
...
# Networking (must match deploy-infra.sh)
...
# Monitoring (must match deploy-infra.sh)
LOG_ANALYTICS_WORKSPACE="${LOG_ANALYTICS_WORKSPACE:-law-tktph-avd-sea}"

# Application URLs (must match deploy-infra.sh)
...
# V9: ActivTrak integration (optional)
...
if ! az network vnet show --resource-group "$RESOURCE_GROUP" --name "$VNET_NAME" &>/dev/null; then
    fail "VNet $VNET_NAME not found in $RESOURCE_GROUP. Run deploy-infra.sh first."
fi
...
if ! az monitor log-analytics workspace show --resource-group "$RESOURCE_GROUP" --workspace-name "$LOG_ANALYTICS_WORKSPACE" &>/dev/null; then
    fail "Log Analytics workspace $LOG_ANALYTICS_WORKSPACE not found. Run deploy-infra.sh first."
fi
...
deploy_phase3_app_rules() {
    ...
    # V9: ActivTrak application rules (optional)
    if [[ -n "$ACTIVTRAK_ACCOUNT_ID" ]]; then
        log INFO "Creating application rules: ActivTrak (V9)..."
        az network firewall policy rule-collection-group collection add-filter-collection \
            --resource-group "$RESOURCE_GROUP" \
            --policy-name "$FIREWALL_POLICY_NAME" \
            --rule-collection-group-name "Business-Applications" \
            --name "Allow-ActivTrak" \
            --collection-priority 250 \
            --action Allow \
            --rule-name "ActivTrak-Cloud" \
            --rule-type ApplicationRule \
            --source-addresses "$SUBNET_PREFIX" \
            --protocols Https=443 \
            --target-fqdns \
                "*.activtrak.com" \
                "*.birchgrove.com" \
            --output none 2>/dev/null || log INFO "ActivTrak rule collection already exists"
        log SUCCESS "ActivTrak firewall rules added"
    fi
